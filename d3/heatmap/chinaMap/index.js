...
d3.json('./china.json').then(function (data) {
    ...
    chart.renderCenter = function () {
        ...
        d3.csv("../../数据集/map_database.csv").then(data => {
            const points = data.map((d) => ({
                x: Math.round(projection([+d.DEC_LAT, +d.DEC_LONG])[0] + config.margins.left),
                y: Math.round(projection([+d.DEC_LAT, +d.DEC_LONG])[1] + config.margins.top),
                value: 1
            }));
